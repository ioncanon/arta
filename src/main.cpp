...
uint256 hashGenesisBlock("0x00000ef3ddfd9a7a0274c4ae9c978537d789c37e39e695fc50821f4b7d7b1248");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ARTA: starting difficulty
...
{
    // ARTA: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // ARTA: 1 minute
static const int64 nTargetSpacing = 1* 60; // ARTA: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ARTA: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 1 * 60; // ARTA: 1 minute
        static const unsigned int TimeDaySeconds = 1 * 60; // ARTA: 1 minute
...
{
    // ARTA: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04661c4fc68647381324f82897a81205beb3ab83feeba9a2e49ecda286fb9cdca69050bd4a1540d83e21485071416bb88437853779f21b388ea90cdd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1490896384;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 372849;

        if (fTestNet)
        {
            block.nTime    = 1490842383;
            block.nNonce   = 325434;
        }
 ...
 if (false && block.GetHash() != hashGenesisBlock)
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
         assert(block.hashMerkleRoot == uint256("0x3583a248ba2a69e9a2833353f0c6dd9748d7622ed72eb82064971f5b6e142907"));
...
unsigned char pchMessageStart[4] = { 0xee, 0xca, 0xab, 0xd7 }; // ARTA: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ARTAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ARTAMiner : ProcessBlock, block not accepted");
    }
